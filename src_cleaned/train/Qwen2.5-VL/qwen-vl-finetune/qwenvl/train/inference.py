...
def inference():
    ...
    main_training_output_dir = lora_checkpoint_path.parent

    print(f"DEBUG: LoRA Checkpoint Path: '{lora_checkpoint_path}'")
    print(f"DEBUG: Main Training Output Dir: '{main_training_output_dir}'")
    print(f"DEBUG: Dataset for inference: '{data_args.dataset_use}'")
    print(f"DEBUG: Output JSON path: '{inference_args.output_path}'")
    ...
    #     base_model, str(lora_checkpoint_path), ephemeral_gpu_offload=True
    ...
    image_processor = Qwen2VLImageProcessor.from_pretrained(
        str(main_training_output_dir),
        trust_remote_code=True
    )
    data_args.image_processor = image_processor

    training_config_path = main_training_output_dir.parent / "configs" / "training_config.json"
    if training_config_path.exists():
        with open(training_config_path, 'r') as f:
            saved = json.load(f).get("data_args", {})
